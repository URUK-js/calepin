...
const setupWSConnection = (conn, req) => {
    ...
    const doc = (0, doc_1.getYDoc)(docName, gc);
    doc.conns.set(conn, new Set());
    // listen and reply to events
    conn.on("message", (message) => (0, messenger_1.messageListener)(conn, doc, new Uint8Array(message)));
    // Check if connection is still alive
    let pongReceived = true;
    const pingInterval = setInterval(() => {
        if (!pongReceived) {
            if (doc.conns.has(conn)) {
                (0, exports.closeConn)(doc, conn);
            }
    ...
    // scope
    {
        ...
        (0, messenger_2.send)(doc, conn, encoding.toUint8Array(encoder));

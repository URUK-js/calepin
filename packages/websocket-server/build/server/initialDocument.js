...
exports.getContent = (leaf) => {
    return exports.createLeaf(leaf);
};
exports.getChildren = (_a) => {
    var { type, content = [], children = [] } = _a, props = __rest(_a, ["type", "content", "children"]);
    return exports.createNode(type, Object.assign(Object.assign({}, props), { children: children.map(exports.getChildren), content: content.map(exports.getContent) }));
};
exports.createLeaf = (props) => {
    const leaf = new yjs_1.Map();
    if (props && props.id) {
        leaf.set("id", props.id);
    }
    else {
        leaf.set("id", "y-" + nanoid_1.nanoid());
    }
    if (props && props.text) {
...
exports.createNode = (type, props) => {
    const node = new yjs_1.Map();
    if (props && props.id) {
        node.set("id", props.id);
    }
    else {
        node.set("id", "y-" + nanoid_1.nanoid());
    }
    node.set("type", type);
    if (props && props.data) {
